...
- **arq worker**（`worker` 容器）：範本轉換 / 克隆、一鍵重置、管理員直接建 VM、班級批次佈建都以 TaskRecord 入列到 Redis 由 worker 執行；任務模組清單在 `infrastructure/queue/modules.py`。REDIS_ENABLED=false 時退回行程內背景執行
