...
- **arq worker**（`worker` 容器）：範本轉換 / 克隆、VM 申請佈建（clone）、一鍵重置、班級批次佈建、資源刪除都以 TaskRecord 入列到 Redis 由 worker 執行；任務模組清單在 `infrastructure/queue/modules.py`。REDIS_ENABLED=false 時退回行程內背景執行。除了申請佈建（申請單本身就是一個 job），這些任務會出現在 Jobs 頁（kind：template / resource_reset / batch_provision）。VM 申請佈建以固定 job id `vm_request:<id>` 去重，並在 worker 內以 `provision_max_concurrency` 限制同時 clone 數
