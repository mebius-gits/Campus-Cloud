...
def _sync_traefik(session: object) -> None:
    ...
    from app.services.gateway_service import (  # noqa: PLC0415
    ...
    try:
        # 確保目錄存在
        _exec(client, f"mkdir -p $(dirname {TRAEFIK_DYNAMIC_PATH})")
        ...
        code, out, err = _exec(client, f"mv {tmp_path} {TRAEFIK_DYNAMIC_PATH}")
        if code != 0:
