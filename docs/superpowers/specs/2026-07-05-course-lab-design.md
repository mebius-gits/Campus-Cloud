...
| 實驗環境型態 | VM + LXC 都支援，統一綁定模板系統 2.0 的 `VMTemplate`（`resource_type` qemu/lxc） |
...
| `CourseRoom` | `id`, `path_id`(FK CASCADE), `title`, `description`, `difficulty`(easy/medium/hard), `category`, `template_id`(FK → `vm_templates.id`), `order` |
...
- `progress_service.py` — 學生自身進度、老師端全班統計
...
1. 驗證：房間屬 published 路徑、`VMTemplate` 為 `ready`、
   使用者無進行中課程部署（`course_max_active_per_user`）。
...
| `pages/courses/room/CourseRoomPage.jsx` | 三欄：任務導航｜Markdown 內容 + Flag 輸入｜內嵌 VNC；頂部啟動按鈕 + 剩餘時間倒數 |
| `pages/teaching/course-cms/CourseCmsPage.jsx` | 樹狀編輯器、發布開關、模板下拉（既有 templates service）、學生進度 tab |
...
- 後端：`flag_service` 純函式單測；`deployment_service` 整合測試（單人單機、published 檢查、
  TTL 寫入）；API 測試（權限 403、flag_hash 不外洩、submit 冪等）。
