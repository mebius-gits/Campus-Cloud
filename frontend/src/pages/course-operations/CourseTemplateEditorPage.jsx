...
function MachineEditor({ value, onChange, pveTemplates }) {
  ...
  return <section className={`${styles.card} ${styles.templateMachineWorkspace}`}>
      ...
      <div className={styles.machineAddBar}>
        <label className={styles.field}><span>新增機器來源</span><select value={sourceId} onChange={(event) => setSourceId(event.target.value)}><option value="">選擇既有 PVE 範本</option>{sources.map((source) => <option key={source.id} value={source.id}>{source.name} · {source.resource_type ?? "VM"}</option>)}</select></label>
        <button type="button" className={styles.btnSecondary} disabled={!sourceId} onClick={addMachine}><MIcon name="add" size={16} />加入機器</button>
      </div>
...
export default function CourseTemplateEditorPage() {
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.stepTabs}>{TABS.map(([key, icon, label], index) => <button type="button" key={key} className={tab === key ? styles.stepActive : ""} onClick={() => changeTab(key)}><span>{index + 1}</span><MIcon name={icon} size={17} />{label}</button>)}</div>
